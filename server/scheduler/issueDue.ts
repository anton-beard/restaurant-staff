...
export async function issueDueTemplates(deps: SchedulerDeps, now: Date): Promise<number> {
  ...
  for (const t of listDueTemplates(deps.db, now.toISOString())) {
    ...
    // сбой одного шаблона (битое расписание, недоступный Telegram) не должен останавливать остальные;
    // next_run_at упавшего не двигаем — он попробует снова на следующем тике
    try {
      ...
      console.error('scheduler: template issue failed', t.id, err)
    }
  }
  return issued
}
