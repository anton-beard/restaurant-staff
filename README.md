...
`npm run seed` наполняет базу вымышленным рестораном: три должности, семь сотрудников, шесть заданий по расписанию с историей за 30 дней, три курса с тестами и еженедельный тест «Меню недели». Повторный запуск ничего не добавляет. Демо-сотрудники привязаны к несуществующим Telegram-аккаунтам, поэтому сообщения им не доставляются, а новые задания по расписанию уходят в просрочку. Чтобы убрать демо, заархивируйте сотрудников в админке или удалите `data/app.db`.

В Docker: `docker compose -f docker-compose.prod.yml exec app npm run seed`.
